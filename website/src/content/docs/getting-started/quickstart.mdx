...
data = pool.get_source_bytes(sources[0]["name"], 256, conditioning="sha256")
  ```
  </TabItem>
  <TabItem label="CLI">
  ```bash
...
pool = EntropyPool.auto()
data = pool.get_source_raw_bytes(source, 5000)
result = full_analysis(source, data)
...
- [Concepts: Entropy Sources](/openentropy/concepts/sources/) — source catalog, platform availability, and physics notes
